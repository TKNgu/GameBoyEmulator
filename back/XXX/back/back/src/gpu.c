...
static enum gb_color gb_gpu_get_tile_color(struct gb *gb, uint8_t tile_index,
                                           uint8_t x, uint8_t y,
                                           bool use_sprite_ts,
                                           bool use_high_bank) {
  unsigned tile_addr;
...
static struct gb_gpu_pixel
gb_gpu_get_bg_win_pixel(struct gb *gb, uint8_t x, uint8_t y, bool use_high_tm) {
  struct gb_gpu *gpu = &gb->gpu;
  ...
  if (gb->gbc) {
    ...
    col = gb_gpu_get_tile_color(gb, tile_index, tile_x, tile_y, use_sprite_ts,
                                high_bank);
    ...
    pix.color.dmg_color = gb_gpu_get_tile_color(gb, tile_index, tile_x, tile_y,
                                                use_sprite_ts, false);
    pix.opaque = pix.color.dmg_color != GB_COL_WHITE;

    pix.color.dmg_color =
        gb_gpu_palette_transform(pix.color.dmg_color, gpu->bgp);
  }

  return pix;
}

static struct gb_gpu_pixel gb_gpu_get_bg_pixel(struct gb *gb, unsigned x,
                                               unsigned y) {
  struct gb_gpu *gpu = &gb->gpu;
...
static struct gb_gpu_pixel gb_gpu_get_win_pixel(struct gb *gb, unsigned x,
                                                unsigned y) {
  struct gb_gpu *gpu = &gb->gpu;
...
static void
gb_gpu_get_line_sprites(struct gb *gb, unsigned ly,
...
static bool gb_gpu_get_sprite_col(struct gb *gb, const struct gb_sprite *sprite,
  ...
  if (gb->gbc) {
    p->color.gbc_color = gpu->sprite_palettes.colors[sprite->palette][col];
  } else {
    uint8_t palette;
