...
uint8_t *MemoryRead1(struct gb *gb, uint16_t addr) {
     if (addr >= ROM_BASE && addr < ROM_END) {
          return CartRomRead(gb, addr - ROM_BASE);
     }
     ...
     if (addr >= CRAM_BASE && addr < CRAM_END) {
          return CartRamRead(gb, addr - CRAM_BASE);
     }
     ...
     if (addr == REG_INPUT) {
          return InputGetState(gb); // gb_input_get_state(gb);
     }
     ...
     if (addr == REG_DIV) {
        ...
        static uint8_t tmp;
            tmp = gb->timer.divider_counter >> 8;
          return &tmp;
     }
     ...
     if (addr == REG_NR11) {
        static uint8_t tmp;
        tmp = (gb->spu.nr1.wave.duty_cycle << 6) | 0x3f;
          return &tmp;
     }

     if (addr == REG_NR12) {
        static uint8_t tmp;
        tmp = gb->spu.nr1.envelope_config;
        return &tmp;
     }
     ...
     if (addr == REG_NR14) {
        static uint8_t tmp;
          tmp = (gb->spu.nr1.duration.enable << 6) | 0xbf;
        return &tmp;
     }

     if (addr == REG_NR21) {
        static uint8_t tmp;
          tmp = (gb->spu.nr2.wave.duty_cycle << 6) | 0x3f;
        return &tmp;
     }

     if (addr == REG_NR22) {
        static uint8_t tmp;
          tmp = gb->spu.nr2.envelope_config;
        return &tmp;
     }
     ...
     if (addr == REG_NR24) {
        static uint8_t tmp;
          tmp =  (gb->spu.nr2.duration.enable << 6) | 0xbf;
        return &tmp;
     }
     ...
     if (addr == REG_NR32) {
        static uint8_t tmp;
          tmp = (gb->spu.nr3.volume_shift << 5) | 0x9f;
        return &tmp;
     }
     ...
     if (addr == REG_NR34) {
        static uint8_t tmp;
        tmp = (gb->spu.nr3.duration.enable << 6) | 0xbf;
        return &tmp;
     }
     ...
     if (addr == REG_NR44) {
        static uint8_t tmp;
          tmp =  (gb->spu.nr4.duration.enable << 6) | 0xbf;
        return &tmp;
     }
     ...
     if (addr == REG_LCDC) {
        static uint8_t tmp;
          tmp = gb_gpu_get_lcdc(gb);
        return &tmp;
     }

     if (addr == REG_LCD_STAT) {
        static uint8_t tmp;
          tmp = gb_gpu_get_lcd_stat(gb);
        return &tmp;
     }
