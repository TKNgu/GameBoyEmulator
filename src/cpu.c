...
void CPU_JP_HL(CPU *cpu) {
    CPU_LOAD_PC(cpu, cpu->hl);
}
...
void CPU_POP_R16stk(CPU *cpu) {
    ...
    if (r16) {
        *r16 = CPU_POPW(cpu);
        ...
        uint8_t f = CPU_POPB(cpu);
        uint8_t a = CPU_POPB(cpu);
...
void CPUResSetFlags(CPU *cpu, uint8_t *v, uint8_t bit) {
     *v = *v & ~(1U << bit);
}

void CPUSetSetFlags(CPU *cpu, uint8_t *v, uint8_t bit) {
     *v = *v | (1U << bit);
}
